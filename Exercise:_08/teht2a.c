...
int main (int argc, char *argv[], char *env[]) {
    ...
    /*kirjataan lipun arvoksi 1*/
    //p->lippu = 1;
    //annetaan prosessi B:n ottaa yhteisalue käyttöön.
    sem_post(mysem_b);

    printf("Syötä henkilötiedot -> tiedot näkyvät kumppaniprosessissa. Poistuaksesi syötteestä, paina q + enter\n");
    while (1) {
        ...
        while (1) {
            ...
            printf("Ole hyvä ja anna henkilön nimi! Poistuaksesi syötteestä, paina q + enter\n");
        }
        ...
        strcpy(p->nimi, puskuri_nimi);
        p->ika = puskuri_ika;
        /*ilmoitetaan B prosessille että voi nyt käsitellä yhteisen alueen tietoja*/
        sem_post(mysem_b);
    }

    /*odotetaan että B prosessi on sulkenu semafori B käytöstä*/
    sem_wait(mysem_a);
